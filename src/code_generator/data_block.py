class Data:
    ...
    def set_keyword(self, keyword):
        if self.type == 'void' and keyword != 'func':
            raise Exception(f'void type can only be used for functions')
...
class DataBlock:
    def __init__(self):
        initial_data = Data('output', '', None, 0)
    ...
    def get_data(self, lexeme: str):
        ...
        for data in self.all_data:
            if data.lexeme == lexeme and (data.scope == scope or data.keyword == 'func'):
                return data

        raise Exception(f'Variable {lexeme} not declared')
